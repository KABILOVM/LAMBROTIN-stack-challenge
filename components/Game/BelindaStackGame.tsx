...
const GameScene = ({ onGameOver, onScoreUpdate, gameState, triggerClick }: GameProps & { triggerClick: React.MutableRefObject<() => void> }) => {
  ...
  useFrame((state) => {
    ...
    // Adjusted lookAt to be higher (totalHeight + 3), which pushes the rendered stack lower on screen
    camLookAt.current.lerp(new THREE.Vector3(0, totalHeight + 3, 0), 0.05);
