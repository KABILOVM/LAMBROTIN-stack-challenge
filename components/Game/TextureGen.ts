...
export const createTopTexture = (width: number, depth: number, color: string) => {
  ...
  if (ctx) {
    ...
    // Снижена интенсивность белого, чтобы цвет был ярче
    const grad = ctx.createRadialGradient(size/2, size/2, size * 0.02, size/2, size/2, size * 0.9);
    grad.addColorStop(0, 'rgba(255,255,255,0.2)'); // Было 0.4
    grad.addColorStop(0.4, 'rgba(255,255,255,0.05)');
    grad.addColorStop(1, 'rgba(255,255,255,0)');
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, size, size);

    // Яркая тонкая обводка
...
export const createSideTexture = (width: number, height: number, color: string, text: string) => {
  ...
  if (ctx) {
    ...
    // Почти прозрачный градиент для сохранения сочности
    const grad = ctx.createLinearGradient(0, 0, 0, texH);
    grad.addColorStop(0, 'rgba(255,255,255,0.15)'); // Было 0.3
    grad.addColorStop(0.5, 'rgba(255,255,255,0)');
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, texW, texH);

    // Четкая белая фаска сверху
    ctx.fillStyle = 'rgba(255,255,255,0.6)';
    ctx.fillRect(0, 0, texW, 4);
    ...
    if (w > 0.4) { 
        ...
        // Поля 60px — текст не обрывается
        const padding = 60;
        const fontSize = fitText(ctx, russianText, texW - padding, 72);
        
        ctx.shadowColor = 'rgba(0,0,0,0.2)';
        ctx.shadowBlur = 6;
        ctx.shadowOffsetY = 1;
        
        ctx.fillText(russianText, texW/2, texH/2 + 6);
    }
  }
