...
// Added missing LockIcon component to fix "Cannot find name 'LockIcon'"
...
export const RegisterScreen = ({ onRegisterSuccess, onAdminLogin, lang, setLang }: { onRegisterSuccess: (isNewUser: boolean) => Promise<void>, onAdminLogin: () => void, lang: Language, setLang: (l: Language) => void }) => {
    ...
    const handleAdminSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        setAdminError('');
        // Secure hardcoded admin password (can be moved to backend in real app)
    ...
    return (
        <div className="w-full min-h-full bg-slate-50 flex flex-col items-center justify-center p-4 md:p-6 antialiased overflow-y-auto">
            <div className="w-full max-w-md bg-white rounded-[40px] md:rounded-[56px] p-8 md:p-10 shadow-2xl shadow-slate-200 border border-slate-100 animate-fade-in flex flex-col items-center relative overflow-y-auto max-h-[90vh] custom-scrollbar mb-4">
                
                {/* Visual Flair: Soft Gradient Blobs */}
