...
export const RegisterScreen = ({ onRegisterSuccess, onAdminLogin, lang, setLang }: { onRegisterSuccess: (isNewUser: boolean) => void, onAdminLogin: () => void, lang: Language, setLang: (l: Language) => void }) => {
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 mb-4">
                ...
                <div className="space-y-4">
                    ...
                    <div className="pt-4 space-y-3">
                        ...
                        <button 
                            ...
                            className="w-full py-5 bg-indigo-50 text-indigo-500 font-black rounded-[25px] border-2 border-indigo-100 uppercase tracking-widest text-xs transition active:scale-95 hover:bg-indigo-100 flex items-center justify-center gap-3"
                        >
            ...
            {showUpload && !uploadSuccess && (
                <div className="space-y-6 animate-fade-in">
                    <div className="text-center p-6 bg-slate-50 rounded-[30px] border-2 border-dashed border-slate-200">
                        <input 
                            type="file" 
                            accept="image/*" 
                            className="hidden" 
                        ...
                        {previewUrl ? (
                            <div className="relative">
                                <img src={previewUrl} className="max-h-64 mx-auto rounded-xl shadow-md" alt="Preview" />
                                <button 
                                    onClick={() => { setPreviewUrl(null); setSelectedFile(null); }}
                                    className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-8 h-8 flex items-center justify-center shadow-lg font-bold"
                                >
                                    &times;
                                </button>
                            </div>
                            ...
                            <div className="flex flex-col items-center gap-4 py-8 cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                                ...
                                <div>
                                    <h3 className="font-bold text-slate-700 uppercase tracking-wider text-sm mb-1">{T.uploadInvoice}</h3>
                                    <p className="text-[10px] text-slate-400 font-medium max-w-[200px] mx-auto leading-relaxed">{T.uploadDesc}</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col gap-3">
                        <button 
                            ...
                            className="w-full py-5 bg-slate-800 text-white font-black rounded-[25px] uppercase shadow-xl shadow-slate-200 transition active:scale-95 hover:bg-slate-900 tracking-widest text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                        >
            ...
            {showUpload && uploadSuccess && (
                <div className="flex flex-col items-center justify-center py-10 animate-fade-in text-center">
                    ...
                    <p className="text-xs text-slate-500 font-medium max-w-xs leading-relaxed mb-8">{T.waitMsg}</p>
                    <button 
                        onClick={() => { setShowUpload(false); setUploadSuccess(false); onClose(); }}
                        className="w-full py-5 bg-slate-100 text-slate-500 font-black rounded-[25px] uppercase tracking-widest text-xs hover:bg-slate-200"
                    >
                        {T.ok}
                    </button>
                </div>
