...
export const AdminPanel = ({ onBack, onTestGame, onPrizesUpdated }: AdminPanelProps) => {
  ...
  const handleCompleteOrder = async (userId: string) => {
    if (!window.confirm("Пометить заказ как выполненный? Статус заказа будет сброшен.")) return;
    setLoading(true);
    try {
              ...
              {activeTab === 'orders' && (
                  <div className="space-y-8 animate-fade-in">
                      <div className="flex items-center justify-between">
                          <h3 className="text-lg font-black text-slate-800 uppercase italic">Запросы на доставку призов</h3>
                          <div className="bg-white px-4 py-2 rounded-xl border border-slate-100 shadow-sm">
                              <span className="text-xs font-black text-slate-800">{deliveryUsers.length}</span> <span className="text-[9px] font-black text-slate-400 uppercase">активных</span>
                          </div>
                      </div>
                      <div className="grid grid-cols-1 gap-6">
                          {deliveryUsers.length === 0 ? (
                              ...
                              deliveryUsers.map(u => (
                                  <div key={u.id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-8 animate-fade-in">
