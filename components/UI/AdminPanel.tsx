            ...
            <div className="w-full max-w-2xl bg-white rounded-[40px] overflow-hidden flex flex-col h-full max-h-[80vh] shadow-2xl">
                ...
                <div className="p-8 space-y-6 bg-white border-t border-slate-50">
                    ...
                    <div className="flex gap-4">
                        <button 
                            onClick={onCancel} 
                            className="flex-1 py-4 bg-slate-100 text-slate-500 font-black rounded-2xl uppercase text-[10px] tracking-widest hover:bg-slate-200 transition"
                        >
                            Отмена
                        </button>
                        <button 
                            ...
                            className="flex-1 py-4 bg-blue-600 text-white font-black rounded-2xl uppercase text-[10px] tracking-widest shadow-xl shadow-blue-100 hover:bg-blue-700 transition"
                        >
                            Применить
                        </button>
                    </div>
...
export const AdminPanel = ({ onBack, onTestGame }: AdminPanelProps) => {
  ...
  const handlePrizeImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const onCropApply = async (croppedBase64: string) => {
      if (!editingPrize) return;
      setLoading(true);
      try {
          const { data } = await compressImage(croppedBase64);
          setEditingPrize({ ...editingPrize, imageUrl: data });
          setImageToCrop(null);
      } catch (e) {
                                            ...
                                            <td className="px-8 py-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center overflow-hidden shrink-0">
                                                        {p.imageUrl ? <img src={p.imageUrl} className="w-full h-full object-cover" /> : <PrizeIcon name={p.icon} className="w-5 h-5 text-slate-400" />}
                                                    </div>
                                                    <div>
              ...
              <div className="bg-white w-full max-w-lg p-10 rounded-[50px] shadow-2xl animate-fade-in relative overflow-hidden max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="space-y-6">
                      <div className="flex flex-col items-center">
                          <label className="block w-full text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 px-1">Фотография приза</label>
                          <input type="file" className="hidden" ref={prizeImageRef} accept="image/*" onChange={handlePrizeImageChange} />
                          
                          {editingPrize.imageUrl ? (
                              <div className="relative group w-32 h-32 rounded-3xl overflow-hidden border-2 border-blue-100 shadow-lg">
                                  <img src={editingPrize.imageUrl} className="w-full h-full object-cover" />
                                  <button 
                                      onClick={() => setEditingPrize({...editingPrize, imageUrl: undefined})}
                                      className="absolute inset-0 bg-red-500/80 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity font-black text-[10px] uppercase"
                                  >
                                      Удалить фото
                                  </button>
                              </div>
                              ...
                              <button 
                                ...
                                className="w-32 h-32 rounded-3xl bg-slate-50 border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-2 hover:bg-blue-50 hover:border-blue-200 transition-all group"
                              >
                                  <div className="p-3 bg-white rounded-xl shadow-sm text-slate-400 group-hover:text-blue-500">
                                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M6.827 6.175A2.31 2.31 0 015.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9.574c0-1.067-.75-1.994-1.802-2.169a47.865 47.865 0 00-1.134-.175 2.31 2.31 0 01-1.64-1.055l-.822-1.316a2.192 2.192 0 00-1.736-1.039 48.774 48.774 0 00-5.232 0 2.192 2.192 0 00-1.736 1.039l-.821 1.316z" /><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 12.75a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0zM18.75 10.5h.008v.008h-.008V10.5z" /></svg>
                                  </div>
                                  <span className="text-[9px] font-black text-slate-400 uppercase group-hover:text-blue-500">Загрузить</span>
                              </button>
                          )}
                          <p className="text-[8px] text-slate-300 font-bold uppercase mt-2">Рекомендуется 1:1, до 500 КБ</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div>
                            <label className="block text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 px-1">Запасная иконка (если нет фото)</label>
