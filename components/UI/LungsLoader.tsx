...
export const LungsLoader = ({ className = "", size = 160, progress = 0 }: LungsLoaderProps) => {
    ...
    // Расчет высоты наполнения (800 - это высота SVG)
        ...
        <div className={`flex flex-col items-center justify-center gap-6 ${className} antialiased transition-all duration-500 ${isComplete ? 'scale-110' : 'scale-100'}`}>
            <div className="relative" style={{ width: size, height: size * (757/800) }}>
                ...
                >
                    ...
                    {/* Наполнение сиропом - динамичный переход */}
                        ...
                        <rect 
                            ...
                            style={{ 
                                ...
                                transition: progress > 90 
                                    ? 'transform 0.25s cubic-bezier(0.175, 0.885, 0.32, 1.275)' // Резкий "прыжок" в конце
                ...
                {/* Центрированный счетчик процентов */}
            ...
            <style>{`
                ...
                .animate-message-fade {
                    animation: messageFade 0.3s ease-out forwards;
                }
            `}</style>
        </div>
    );
