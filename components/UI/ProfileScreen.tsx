...
export const ProfileScreen = ({ user: initialUser, onLogout, lang, setLang, prizes, onPlayCode, initialTab = 'prizes', initialCodesSubTab = 'get' }: ProfileScreenProps) => {
  ...
  // Устанавливаем текущий открытый тир в зависимости от закупок пользователя
  const [activePrizeTier, setActivePrizeTier] = useState<PrizeTier>(
    initialUser.maxPurchaseTier === 'BASIC' ? 'BRONZE' : initialUser.maxPurchaseTier
  );
        ...
        >
            ...
            <div className="flex-1 space-y-1">
                ...
                {prize.id === 'TIER_1' && isUnlocked && !isConfirmed && (
                    <span className="text-[8px] font-black uppercase bg-blue-600 text-white px-1.5 py-0.5 rounded-md inline-block">Автоматически</span>
                )}
                {state === 'locked_tier' ? (
                ...
                <div className="mt-auto pt-2">
                    ...
                    <div className="flex justify-between items-center">
                         <span className={`text-[9px] font-black uppercase tracking-widest ${state === 'confirmed' ? 'text-white/80' : 'text-slate-400'}`}>
                            {prize.id === 'TIER_1' ? 'Асосӣ' : (isUnlockedByScore ? 'Дастрас' : `${Math.floor(progressPercent)}%`)}
                         </span>
  ...
  // Умная фильтрация: исключаем базовую карту, если в текущем тире есть своя премиальная карта
  const filteredPrizes = prizes
    .filter(p => {
        if (p.tier === activePrizeTier) return true;
        
        // Если приз базовый (BASIC), проверяем, нет ли в текущем открытом тире аналога с такой же иконкой
        if (p.tier === 'BASIC') {
