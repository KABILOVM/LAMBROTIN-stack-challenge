    ...
    return (
        ...
        >
            ...
            <div className="flex-1">
                <h4 className={`text-sm font-black tracking-tight leading-tight mb-1 ${state === 'confirmed' ? 'text-white' : 'text-slate-800'}`}>
                    {prize.title}
                </h4>
                ...
                {state === 'locked_tier' ? (
                   <p className="text-[10px] font-black text-blue-500 bg-blue-50/50 p-2 rounded-lg mt-1 border border-blue-100">
                   ...
                   <p className="text-[9px] font-black text-slate-400 mt-1 italic">
                    ...
                    <p className={`text-[10px] font-bold leading-normal line-clamp-2 ${state === 'confirmed' ? 'text-white/80' : 'text-slate-400'}`}>{prize.description}</p>
                )}
            </div>
            {!isConfirmed && (
                <div className="mt-auto">
                    ...
                    <div className="flex justify-between items-center">
                         <span className={`text-[9px] font-black ${state === 'confirmed' ? 'text-white/80' : 'text-slate-400'}`}>
                            {prize.id === 'TIER_1' ? 'Обязательный' : (isUnlockedByScore ? T.valuablePrize : `${Math.floor(progressPercent)}%`)}
                         </span>
                         <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black ${state === 'confirmed' ? 'bg-white/20' : 'bg-slate-50 text-slate-500 border border-slate-100'}`}>
    ...
    <div className="w-full h-full bg-slate-50 antialiased selection:bg-blue-100 relative">
                    ...
                    <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 flex flex-col gap-8">
                        <div className="flex justify-between items-center pb-8 border-b border-slate-100">
                            ...
                            <div className="flex bg-slate-50 p-1 rounded-full border border-slate-200 shadow-inner">
                                <button onClick={() => setLang('ru')} className={`px-6 py-2 rounded-full text-[10px] font-black transition-all ${lang === 'ru' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-400'}`}>RU</button>
                                <button onClick={() => setLang('tg')} className={`px-6 py-2 rounded-full text-[10px] font-black transition-all ${lang === 'tg' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-400'}`}>TJ</button>
                            </div>
                        </div>
      ...
      {showDeliveryConfirmation && (
          <div className="fixed inset-0 z-[160] flex items-center justify-center bg-slate-900/60 p-6 backdrop-blur-sm antialiased" onClick={() => setShowDeliveryConfirmation(false)}>
              <div className="bg-white w-full max-w-sm p-10 rounded-[48px] shadow-2xl animate-fade-in relative text-center border border-slate-100 overflow-hidden" onClick={e => e.stopPropagation()}>
