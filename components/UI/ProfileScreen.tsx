...
export const ProfileScreen = ({ user: initialUser, onBack, onLogout, embedded = false, lang, setLang, prizes, onPlayCode, initialTab = 'prizes', muted, toggleMute }: ProfileScreenProps) => {
  ...
  // Dynamic settings
  const [orderPhone, setOrderPhone] = useState('+992000000000');
  ...
  const allCodesUsed = sortedCodes.length > 0 && sortedCodes.every(c => c.isUsed);
  const noCodesAtAll = sortedCodes.length === 0;
        ...
        <div className={`max-w-xl mx-auto flex flex-col p-6 min-h-full`}>
                ...
                <div className="space-y-4 animate-fade-in">
                    ...
                    {codesSubTab === 'my' ? (
                        <div className="space-y-3">
                            {noCodesAtAll ? (
                                <div className="text-center py-12 px-6 bg-white rounded-[35px] border border-dashed border-slate-200 flex flex-col items-center gap-6">
                                    <p className="text-slate-400 font-black uppercase text-[11px] tracking-widest">{T.noCodes}</p>
                                    <button 
                                        onClick={() => setCodesSubTab('get')}
                                        className="bg-blue-600 text-white px-8 py-4 rounded-[25px] font-black uppercase text-[10px] tracking-[0.2em] shadow-xl shadow-blue-100 flex items-center gap-2 active:scale-95 transition-all"
                                    >
                                        <BoltIcon />
                                        {T.getCodes}
                                    </button>
                                        ...
                                        return (
                                            <div key={c.code} className={`p-4 rounded-[20px] flex justify-between items-center border ${c.isUsed ? 'bg-slate-50 border-slate-100' : 'bg-white border-blue-100 shadow-sm'}`}>
                                                ...
                                                {!c.isUsed && !user.deliveryRequested && (
                                                    <button onClick={() => onPlayCode(c.code)} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold uppercase text-[9px] tracking-widest shadow-lg shadow-blue-200 active:scale-95 transition">{T.playCode}</button>
                                                )}
                                            </div>
                                        );
                                    })}
                                    {allCodesUsed && (
                                        <div className="p-8 bg-blue-50/50 rounded-[35px] border border-blue-100/50 text-center flex flex-col items-center gap-6 animate-fade-in mt-4">
                                            <p className="text-[10px] font-black uppercase text-blue-500 tracking-[0.2em]">Все коды использованы!</p>
                                            <button 
                                                onClick={() => setCodesSubTab('get')}
                                                className="bg-blue-600 text-white px-8 py-4 rounded-[25px] font-black uppercase text-[10px] tracking-[0.2em] shadow-xl shadow-blue-100 flex items-center gap-2 active:scale-95 transition-all"
                                            >
                        ...
                        <div className="space-y-6">
                            {!uploadSuccess && (
                                <>
                                    ...
                                    <a href={`tel:${orderPhone}`} className="w-full py-4 bg-gradient-to-r from-pink-500 to-rose-500 text-white font-black rounded-[20px] shadow-lg shadow-pink-200 uppercase tracking-widest text-xs flex items-center justify-center gap-2 active:scale-95 transition-all">
                                        <PhoneButtonIcon /> {T.orderBtn}
                                    </a>
                                    <div className="my-4 p-3 bg-indigo-50 rounded-xl border border-indigo-100">
                                        <p className="text-[10px] font-bold text-indigo-800 text-center leading-relaxed uppercase">{T.alreadyBoughtHint}</p>
                                    </div>
                                    ...
                                    <button onClick={handleUpload} disabled={!selectedFile || loading} className="w-full py-4 bg-slate-800 text-white font-black rounded-[20px] uppercase shadow-lg shadow-slate-200 transition active:scale-95 hover:bg-slate-900 tracking-widest text-xs disabled:opacity-50">{loading ? T.loading : T.sendPhoto}</button>
                                    <div className="mt-6">
                                        ...
                                        <div className="space-y-2">
                                            {userRequests.map(req => (
                                                <div key={req.id} className="bg-white p-3 rounded-[16px] border border-slate-100 flex items-center justify-between">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 bg-slate-100 rounded-lg overflow-hidden cursor-pointer" onClick={() => setFullScreenImage(req.photoData)}>
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        {req.status === 'approved' && req.codesIssued && req.codesIssued > 0 && <div className="text-[9px] font-bold text-green-600 mt-1">+{req.codesIssued} {getPlural(req.codesIssued, lang)}</div>}
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                            ...
                            {uploadSuccess && (
                                <div className="flex flex-col items-center justify-center py-10 animate-fade-in text-center bg-green-50 rounded-[30px] border border-green-100">
                                    ...
                                    <p className="text-xs text-slate-500 font-medium max-w-xs leading-relaxed mb-6 px-6">{T.waitMsg}</p>
                                    <button onClick={() => { setUploadSuccess(false); setCodesSubTab('my'); }} className="py-3 px-8 bg-white text-slate-500 font-black rounded-[16px] uppercase tracking-widest text-[10px] shadow-sm">{T.ok}</button>
                                </div>
                            )}
                        </div>
                    )}
            ...
            {activeTab === 'settings' && (
                <div className="space-y-4 animate-fade-in">
                    ...
                    <button onClick={onLogout} className="w-full py-5 bg-red-50 text-red-500 font-bold rounded-[25px] border border-red-100 uppercase tracking-widest text-xs hover:bg-red-100 active:scale-95 flex items-center justify-center gap-2">
                    ...
                    <div className="max-w-xl mx-auto pointer-events-auto">
                        <div className="bg-white/80 backdrop-blur-md p-2 rounded-[24px] shadow-lg border border-white/50">
                            {user.deliveryRequested ? (
                                <div className="bg-green-500 text-white p-4 rounded-[20px] text-center font-bold uppercase text-xs flex items-center justify-center gap-2"><CheckIcon /> {T.reqAccepted}</div>
                            ) : (
                                <div className="flex gap-2">
                                    {hasUnsavedChanges && <button onClick={handleConfirmSelection} disabled={loading} className="flex-1 bg-slate-800 text-white py-3.5 rounded-[20px] font-bold uppercase text-[10px] tracking-widest hover:bg-slate-900 shadow-md">{loading ? '...' : T.saveChanges}</button>}
                                    {canRequestDelivery && !hasUnsavedChanges && <button onClick={() => setShowDeliveryConfirmation(true)} disabled={loading} className="flex-1 bg-blue-600 text-white py-3.5 rounded-[20px] font-bold uppercase text-[10px] tracking-widest hover:bg-blue-700 shadow-md shadow-blue-200">{loading ? '...' : T.requestDelivery}</button>}
                                    {!hasUnsavedChanges && !canRequestDelivery && <div className="w-full text-center py-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Выберите призы для сохранения</div>}
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {showDeliveryConfirmation && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/60 p-6 backdrop-blur-sm">
                    <div className="bg-white w-full max-sm p-8 rounded-[40px] shadow-2xl animate-fade-in relative text-center">
                         <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-6">⚠️</div>
                        ...
                        <div className="flex flex-col gap-3">
                            <button onClick={confirmDelivery} disabled={loading} className="w-full py-4 bg-slate-200 text-slate-500 font-bold rounded-[20px] uppercase text-[10px] tracking-widest">{loading ? '...' : T.confirmFinal}</button>
                            <button onClick={() => setShowDeliveryConfirmation(false)} className="w-full py-4 bg-blue-600 text-white font-bold rounded-[20px] shadow-lg shadow-blue-200 uppercase text-[10px] tracking-widest active:scale-95">{T.keepPlaying}</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/60 p-6 backdrop-blur-sm" onClick={() => setShowRules(false)}>
                    <div className="bg-white w-full max-w-md p-8 rounded-[40px] shadow-2xl animate-fade-in" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-4 text-xs font-medium text-slate-600 leading-relaxed">
                            <p>{T.mechanicsDesc}</p>
                            <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100"><p className="text-blue-800">{T.importantDesc}</p></div>
            ...
            {fullScreenImage && (
                <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-4 cursor-zoom-out animate-fade-in" onClick={() => setFullScreenImage(null)}>
                    <img src={fullScreenImage} className="max-w-full max-h-full object-contain rounded-lg shadow-2xl" alt="Fullscreen" />
                    <button onClick={() => setFullScreenImage(null)} className="absolute top-6 right-6 text-white bg-white/20 hover:bg-white/40 rounded-full w-10 h-10 flex items-center justify-center backdrop-blur-sm">&times;</button>
                </div>
            )}
        </div>
      </div>
