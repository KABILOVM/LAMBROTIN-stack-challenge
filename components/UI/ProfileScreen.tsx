...
export const ProfileScreen = ({ user: initialUser, onLogout, lang, setLang, prizes, onPlayCode, initialTab = 'prizes', initialCodesSubTab = 'get' }: ProfileScreenProps) => {
  ...
  // Handle prop updates for tab navigation
  useEffect(() => {
  ...
  const handleToggleSelection = (prize: PrizeConfig) => {
    ...
    if (prize.id === 'TIER_1') return;

    if (user.claimedPrizes?.includes(prize.title)) return; 

    setLocalSelection(prev => {
        const isSelected = prev.includes(prize.title);
        const yovarTitle = prizes.find(p => p.id === 'TIER_1')?.title;
        const otherConfirmedPrize = user.claimedPrizes.find(p => p !== yovarTitle);

        if (otherConfirmedPrize) {
            showTempError("Вы уже получили свой ценный приз!");
            return prev;
        }
        ...
        if (isSelected) {
            ...
            const baseSelection = yovarTitle && prev.includes(yovarTitle) ? [yovarTitle] : [];
            return [...baseSelection, prize.title];
        }
    });
  };
  ...
  const confirmDelivery = async () => {
     setLoading(true);
     try {
         // Automatically claim any locally selected prizes that aren't claimed yet
  ...
  const PrizeCard: React.FC<{ prize: PrizeConfig }> = ({ prize }) => {
    ...
    if (prize.isOutOfStock && !isConfirmed) state = 'outOfStock';
        ...
        const yovarTitle = prizes.find(p => p.id === 'TIER_1')?.title;
        const otherSelected = localSelection.some(p => p !== yovarTitle && p !== prize.title);
        const otherConfirmed = user.claimedPrizes.some(p => p !== yovarTitle);
        
        if (prize.id !== 'TIER_1' && (otherSelected || otherConfirmed)) state = 'locked_limit';
        else state = 'unlocked';
        ...
        >
            <div className="flex justify-between items-start">
                ...
                {state === 'confirmed' ? <div className="bg-white/20 p-1 rounded-full"><CheckIcon className="w-4 h-4 text-white" /></div> : 
                 ...
                 (state === 'locked' || state === 'locked_tier' || state === 'locked_limit') ? <div className="p-1"><LockIcon className="text-slate-300" /></div> : null}
            </div>
            <div className="flex-1">
                <h4 className={`text-sm font-bold tracking-tight leading-tight mb-1 ${state === 'confirmed' ? 'text-white' : 'text-slate-800'}`}>
                    {prize.title}
                </h4>
                {prize.id === 'TIER_1' && isUnlocked && !isConfirmed && (
                    <span className="text-[8px] font-black uppercase bg-blue-600 text-white px-1.5 py-0.5 rounded-md inline-block mb-1">Автоматически</span>
                )}
                {state === 'locked_tier' ? (
                ...
                <div className="mt-auto">
                    ...
                    <div className="flex justify-between items-center">
                         <span className={`text-[9px] font-semibold ${state === 'confirmed' ? 'text-white/80' : 'text-slate-400'}`}>
                            {prize.id === 'TIER_1' ? 'Обязательный' : (isUnlockedByScore ? T.valuablePrize : `${Math.floor(progressPercent)}%`)}
                         </span>
  ...
  return (
    <div className="w-full h-full bg-slate-50 antialiased selection:bg-blue-100 relative">
      {tempError && (
          <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[200] bg-slate-900/90 backdrop-blur-md text-white px-6 py-3 rounded-2xl text-[11px] font-bold uppercase tracking-widest shadow-2xl animate-fade-in border border-white/10">
              {tempError}
          </div>
      )}
            ...
            <div className="max-w-2xl mx-auto flex flex-col gap-4">
                <div className="flex justify-between items-center bg-white p-4 rounded-[28px] shadow-sm border border-slate-100">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-slate-800 rounded-xl flex items-center justify-center text-white text-sm font-light shadow-sm relative">
                            ...
                            <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white flex items-center justify-center text-[7px] font-bold text-white shadow-sm ${
                        ...
                        <div>
                            <div className="text-sm font-bold text-slate-800 tracking-tight leading-none">{user.name}</div>
                    ...
                    <div className="bg-blue-50 px-4 py-2 rounded-xl border border-blue-100 text-center">
                        <div className="text-[8px] font-bold text-blue-400 mb-0.5 uppercase tracking-wide">{T.record}</div>
                        <div className="text-lg font-bold text-blue-700 leading-none">{bestScore}</div>
                    </div>
                </div>
                <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 gap-1">
                    {(['prizes', 'codes', 'history', 'settings'] as const).map(tab => (
                        <button 
                            ...
                            className={`flex-1 py-2.5 rounded-xl text-[9px] uppercase tracking-widest font-bold transition-all whitespace-nowrap ${
                                activeTab === tab 
                ...
                {activeTab === 'codes' && (
                    <div className="flex p-1 bg-white rounded-xl shadow-sm border border-slate-100 animate-fade-in">
                        <button onClick={() => setCodesSubTab('get')} className={`flex-1 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all ${codesSubTab === 'get' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400'}`}> {T.subTabGetCodes} </button>
                        <button onClick={() => setCodesSubTab('my')} className={`flex-1 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all ${codesSubTab === 'my' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400'}`}> {T.subTabMyCodes} </button>
                    </div>
                )}
            </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto flex flex-col p-6 min-h-full">
                ...
                <div className="space-y-6 animate-fade-in pb-12">
                    <div className={`p-6 rounded-[28px] border transition-all ${isTierUnlocked(activePrizeTier) ? 'bg-white border-white shadow-sm' : 'bg-red-50 border-red-100 shadow-inner'}`}>
                        ...
                        {!isTierUnlocked(activePrizeTier) && (
                            <button onClick={() => setActiveTab('codes')} className="mt-4 w-full py-3 bg-red-500 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-lg shadow-red-100 active:scale-[0.98] transition-all">
                                {T.getCodes}
                            </button>
                        )}
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        {filteredPrizes.length > 0 ? filteredPrizes.map(p => <PrizeCard key={p.id} prize={p} />) : <div className="col-span-2 py-10 text-center text-xs font-medium text-slate-300"> Призы в этой категории скоро появятся </div>}
                    </div>
                    <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 mb-20">
                        <Countdown lang={lang} />
                        <p className="mt-4 text-[11px] font-medium text-slate-400 leading-normal text-center"> {T.prizeLimitInfo} </p>
                    </div>
                </div>
            )}
                                    ...
                                    <div key={c.code} className={`p-6 rounded-[35px] flex flex-col md:flex-row gap-4 md:items-center justify-between border transition-all duration-300 ${c.isUsed ? 'bg-slate-50 border-slate-100 grayscale opacity-60' : 'bg-white border-blue-100 shadow-sm hover:shadow-md'}`}>
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {c.purchaseAmount && (
                                                    <span className="text-[10px] font-bold text-slate-400">{c.purchaseAmount.toLocaleString()} смн</span>
                                                )}
                                            </div>
                                        </div>
                        ...
                        <div className="space-y-6 animate-fade-in">
                            {uploadSuccess ? (
                                <div className="bg-white p-10 rounded-[40px] shadow-2xl text-center animate-fade-in relative border border-slate-100 flex flex-col items-center gap-6">
                                    <div className="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-3xl flex items-center justify-center shadow-inner"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={3.5} className="w-10 h-10"><path d="M4.5 12.75l6 6 9-13.5" /></svg></div>
                                    <h3 className="text-lg font-bold text-slate-800 uppercase italic">Заявка отправлена</h3>
                                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100"><p className="text-sm font-medium text-slate-600 leading-relaxed">{T.waitMsg}</p></div>
                                    <button onClick={() => setUploadSuccess(false)} className="w-full py-4 bg-slate-800 text-white font-bold rounded-2xl shadow-lg text-[10px] uppercase tracking-widest transition active:scale-[0.98] hover:bg-slate-900">Понятно</button>
                                </div>
                            ) : (
                                <>
                                    <div className="bg-white p-6 rounded-[28px] border border-slate-100 shadow-sm">
                                        ...
                                        <div className="mt-6 flex items-start gap-3 p-4 bg-amber-50 rounded-2xl border border-amber-100 shadow-inner"><div className="bg-amber-100 text-amber-600 p-1.5 rounded-lg shrink-0"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12v-.008z" /></svg></div><p className="text-[11px] font-bold text-amber-800 leading-normal">{T.tiersIncentive}</p></div>
                                    </div>
                                    <div className="flex flex-col gap-4">
                                        <a href={`tel:${orderPhone}`} className="w-full py-4 bg-slate-800 text-white font-bold rounded-2xl shadow-lg uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 active:scale-95 transition-all"> <BoltIcon /> {T.orderBtn} </a>
                                        <div className="bg-blue-50 border border-blue-100 p-4 rounded-2xl flex items-center gap-3 shadow-sm mx-1"><div className="bg-blue-600 text-white p-2 rounded-lg shrink-0"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg></div><p className="text-[12px] font-bold text-blue-700 leading-tight">{T.alreadyBoughtHint}</p></div>
                                    </div>
                                    <div className="bg-white p-6 rounded-[28px] border border-slate-100 text-center relative overflow-hidden shadow-sm">
                                        ...
                                        {previewUrl ? (
                                            <div className="relative inline-block mb-4"><img src={previewUrl} className="max-h-40 rounded-xl shadow-lg border border-slate-50" alt="Preview" /><button onClick={() => { setPreviewUrl(null); setSelectedFile(null); }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-7 h-7 flex items-center justify-center shadow-lg border-2 border-white">&times;</button></div>
                                        ) : (
                                            <div className="flex items-center gap-4 py-4 px-5 bg-blue-50/50 rounded-2xl border border-dashed border-blue-200 cursor-pointer group mb-4 transition-all hover:bg-blue-50" onClick={() => fileInputRef.current?.click()}><div className="w-10 h-10 bg-white text-blue-500 rounded-xl flex items-center justify-center group-hover:bg-blue-50 transition-colors shadow-sm"><CameraIcon /></div><div className="text-left"><p className="text-[11px] text-slate-500 font-semibold leading-normal">{T.uploadDesc}</p></div></div>
                                        )}
                                        {selectedFile && <button onClick={handleUpload} disabled={loading} className={`w-full py-4 font-bold rounded-2xl shadow-lg text-[10px] uppercase tracking-widest active:scale-[0.98] transition-all ${uploadSuccess ? 'bg-emerald-500 text-white' : 'bg-blue-600 text-white shadow-blue-100'}`}> {loading ? T.loading : (uploadSuccess ? T.reqAccepted : T.sendPhoto)} </button>}
                                    </div>
                                </>
                            )}
                                    ...
                                    <div className="space-y-3 pb-20">
                                        {userRequests.map((req) => (
                                            <div key={req.id} className="bg-white p-4 rounded-2xl flex items-center justify-between border border-slate-100 shadow-sm">
                                                <div className="flex items-center gap-3"><div className="w-10 h-10 bg-slate-50 rounded-xl overflow-hidden border border-slate-100 cursor-pointer transition-transform hover:scale-105" onClick={async () => { const photo = await backend.getRequestPhoto(req.id); if (photo) setFullScreenImage(photo); }}><div className="w-full h-full flex items-center justify-center text-slate-300"><CameraIcon /></div></div><div><div className="text-[11px] font-bold text-slate-700">{new Date(req.createdAt).toLocaleDateString()}</div><div className="text-[9px] font-medium text-slate-400">{new Date(req.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div></div></div>
            ...
            {activeTab === 'history' && (
                <div className="space-y-4 animate-fade-in pb-12">
                    {gameHistory.length === 0 ? <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200 shadow-inner"><p className="text-slate-400 font-bold text-sm">{T.noGames}</p></div> : (
                        gameHistory.map(g => (
                            <div key={g.id} className="bg-white p-5 rounded-2xl border border-slate-50 shadow-sm flex justify-between items-center transition-transform active:scale-[0.98]">
                                <div>
                                    <div className="flex items-center gap-3 mb-1.5"><span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${g.codeUsed === 'TRIAL' ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'}`}>{g.codeUsed === 'TRIAL' ? T.trialGame : g.codeUsed}</span><span className="text-[10px] text-slate-300 font-mono">{new Date(g.playedAt).toLocaleDateString()}</span></div>
                                    <div className="text-2xl font-black text-slate-800 tracking-tighter">{g.score} <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest ml-1">{T.points}</span></div>
                                </div>
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white ${g.score >= 10 ? 'bg-emerald-500 shadow-lg shadow-emerald-50' : 'bg-slate-200 shadow-inner'}`}>
                                    {g.score >= 10 ? <CheckIcon className="w-6 h-6" /> : <span className="font-bold text-xs">!</span>}
                                </div>
                            </div>
                        ))
                    )}
            ...
            {activeTab === 'settings' && (
                <div className="space-y-6 animate-fade-in pb-12">
                    <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 flex flex-col gap-6">
                        <div className="flex justify-between items-center pb-6 border-b border-slate-50">
                            <div>
                                <h4 className="text-sm font-bold text-slate-800">{T.language}</h4>
                                <p className="text-[11px] text-slate-400 mt-1 uppercase tracking-tight">Забони барнома / Язык</p>
                            </div>
                            <div className="flex bg-slate-50 p-1 rounded-full border border-slate-100">
                                <button onClick={() => setLang('ru')} className={`px-5 py-2 rounded-full text-[10px] font-bold transition-all ${lang === 'ru' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-300'}`}>RU</button>
                                <button onClick={() => setLang('tg')} className={`px-5 py-2 rounded-full text-[10px] font-bold transition-all ${lang === 'tg' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-300'}`}>TJ</button>
                            </div>
                        </div>
                        <button onClick={onLogout} className="w-full py-4 bg-red-50 text-red-500 font-bold rounded-2xl text-[10px] uppercase tracking-widest hover:bg-red-100 transition shadow-sm shadow-red-50"> {T.logout} </button>
                    </div>
                </div>
            )}
        </div>
      ...
      {/* Floating Direct Order Button - Appears only when user has chosen a prize */}
      {activeTab === 'prizes' && canDirectOrder && !user.deliveryRequested && (
          <div className="fixed bottom-24 left-0 right-0 z-50 flex justify-center px-6 animate-fade-in">
              <button 
                ...
                disabled={loading}
                className="w-full max-w-sm py-5 bg-blue-600 text-white font-black rounded-[24px] uppercase text-[12px] tracking-[0.15em] shadow-2xl shadow-blue-400/40 transition-all active:scale-95 hover:bg-blue-700 flex items-center justify-center gap-3 border-4 border-white/20"
              >
                  <CheckIcon className="w-5 h-5" />
                  {T.requestDelivery}
              </button>
      ...
      {/* Accepted Status Overlay */}
      {activeTab === 'prizes' && user.deliveryRequested && (
          <div className="fixed bottom-24 left-0 right-0 z-50 flex justify-center px-6 animate-fade-in">
              <div className="w-full max-w-sm py-5 bg-emerald-500 text-white font-black rounded-[24px] uppercase text-[12px] tracking-[0.15em] shadow-xl flex items-center justify-center gap-3 border-4 border-white/20">
                  <CheckIcon className="w-5 h-5" />
                  {T.reqAccepted}
              </div>
          </div>
      ...
      {showDeliveryConfirmation && (
          <div className="fixed inset-0 z-[160] flex items-center justify-center bg-slate-900/60 p-6 backdrop-blur-sm antialiased" onClick={() => setShowDeliveryConfirmation(false)}>
              <div className="bg-white w-full max-w-sm p-8 rounded-[40px] shadow-2xl animate-fade-in relative text-center border border-slate-100 overflow-hidden" onClick={e => e.stopPropagation()}>
                  <div className="absolute top-0 left-0 w-full h-1 bg-blue-500"></div>
                  <div className="w-16 h-16 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-sm border border-blue-100"><BoltIcon className="w-7 h-7" /></div>
                  <h3 className="text-xl font-bold text-slate-800 mb-6 leading-tight uppercase italic">{T.attention}</h3>
                  <div className="text-[13px] font-medium text-slate-600 leading-relaxed mb-10 space-y-4">
                      <p className="font-bold text-slate-800">{T.deliveryWarning}</p>
                      <p>{T.deliveryHint}</p>
                      <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 text-[11px] font-bold text-blue-700 italic">{T.deliveryAdvice}</div>
                  </div>
                  <div className="flex flex-col gap-3">
                      <button onClick={confirmDelivery} disabled={loading} className="w-full py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-xl shadow-blue-100 text-[10px] uppercase tracking-widest hover:bg-blue-700 transition active:scale-[0.98]">{loading ? '...' : T.confirmFinal}</button>
                      <button onClick={() => setShowDeliveryConfirmation(false)} className="w-full py-4 bg-slate-100 text-slate-400 font-bold rounded-2xl text-[10px] uppercase tracking-widest hover:bg-slate-200 transition">{T.keepPlaying}</button>
                  </div>
              </div>
          </div>
      )}
      ...
      {fullScreenImage && (
          <div className="fixed inset-0 z-[200] bg-slate-950/95 flex items-center justify-center p-6 cursor-zoom-out animate-fade-in" onClick={() => setFullScreenImage(null)}>
              ...
              <button onClick={() => setFullScreenImage(null)} className="absolute top-10 right-10 text-white bg-white/10 hover:bg-white/20 rounded-full w-12 h-12 flex items-center justify-center backdrop-blur-md transition text-2xl font-light">&times;</button>
          </div>
      )}
    </div>
  );
