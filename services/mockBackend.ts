    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                // Max width/height 1024px to reduce size
                ...
                // JPEG with 0.6 quality for good compression (~100-300kb)
...
class MockBackendService {
  ...
  async checkUserNotifications(userId: string): Promise<CodeRequest[]> {
      // Find requests updated in last 24h that are resolved
      // Ideally this would use a 'viewed' flag in DB, simplified here by fetching recent resolved
      const { data, error } = await supabase
        .from('code_requests')
        .select('*')
        .eq('user_id', userId)
      ...
      // Simple logic: returns last 3 processed requests. 
      // Real app would filter by !viewed.
      return data.map((r: any) => ({
          id: r.id,
          userId: r.user_id,
          photoData: r.photo_data,
