...
export const compressImage = async (input: File | string): Promise<{data: string, sizeKb: number}> => {
...
class MockBackendService {
  ...
  // Merges database prizes with INITIAL_PRIZES constants
  private mergePrizes(dbPrizes: any[]): PrizeConfig[] {
    ...
    // 2. Override with DB data if exists
  ...
  async getLeaderboard(limit: number = 5): Promise<{name: string, score: number, city: string}[]> {
    const { data, error } = await supabase
      ...
      .limit(limit * 3); // Get extra to handle duplicates from same user
  ...
  async getAdminStats() {
    ...
    const { data: codes } = await supabase.from('promo_codes').select('code, is_used, is_issued, assigned_to, generated_at, invoice_number, purchase_amount').order('generated_at', { ascending: false });
  ...
  async issueCodesToUser(userId: string, qty: number, invoice: string, amount: number, selectedTier?: PrizeTier) {
      // Manual tier assignment from admin panel
  ...
  async getCodeRequests(status: 'pending' | 'approved' | 'rejected' | 'all' = 'pending'): Promise<CodeRequest[]> {
      let query = supabase.from('code_requests').select('id, user_id, status, admin_comment, invoice_number, purchase_amount, codes_issued, created_at, photo_size_kb');
      if (status !== 'all') {
