...
class MockBackendService {
  ...
  private calculateTier(amount: number): PrizeTier {
      if (amount >= 100000) return 'DIAMOND';
      if (amount >= 40000) return 'GOLD';
  ...
  async issueCodesToUser(userId: string, qty: number, invoice: string, amount: number) {
      // Calculate and update user tier based on purchase
      const newTier = this.calculateTier(amount);
      ...
      if (tiers.indexOf(newTier) > tiers.indexOf(currentTier)) {
          await supabase.from('users').update({ max_purchase_tier: newTier }).eq('id', userId);
      }
