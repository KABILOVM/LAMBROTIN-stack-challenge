...
export const compressImage = async (input: File | string, cropToSquare: boolean = false): Promise<{data: string, sizeKb: number}> => {
    return new Promise((resolve, reject) => {
        const processDataUrl = (dataUrl: string) => {
            ...
            img.onload = () => {
                ...
                if (cropToSquare) {
                    ...
                    width = size;
                    height = size;
                    canvas.width = 512; // Standard size for prize icons
                    canvas.height = 512;
                } else {
                ...
                if (cropToSquare) {
                    ctx.drawImage(img, offsetX, offsetY, width, height, 0, 0, 512, 512);
                } else {
                    ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
                }

                const base64 = canvas.toDataURL('image/jpeg', 0.7);
                const sizeKb = Math.round((base64.length * 0.75 / 1024) * 100) / 100;
                resolve({ data: base64, sizeKb });
            };
        ...
        if (typeof input === 'string') {
            ...
            reader.onload = (event) => processDataUrl(event.target?.result as string);
...
class MockBackendService {
  ...
  private mapUserFromDB(u: any): User {
    return {
      ...
      maxPurchaseTier: (u.max_purchase_tier as PrizeTier) || 'BASIC',
      isEmployee: u.is_employee || false
    };
  }

  private mergePrizes(dbPrizes: any[]): PrizeConfig[] {
    const prizesMap = new Map<string, PrizeConfig>();
    INITIAL_PRIZES.forEach(p => prizesMap.set(p.id, p));
    if (dbPrizes && dbPrizes.length > 0) {
        dbPrizes.forEach(p => {
            prizesMap.set(p.id, {
                ...
                tier: (p.tier as PrizeTier) || 'BASIC',
                imageUrl: p.image_url
            });
        });
    }
    return Array.from(prizesMap.values());
  }

  async getPrizes(): Promise<PrizeConfig[]> {
    const { data, error } = await supabase.from('prize_configs').select('*');
    if (error) {
        console.error("Supabase Prize Fetch Error:", error);
        return INITIAL_PRIZES;
    }
    return this.mergePrizes(data || []);
  }
  ...
  async setUserEmployeeStatus(userId: string, isEmployee: boolean) {
    const { error } = await supabase.from('users').update({ is_employee: isEmployee }).eq('id', userId);
    if (error) throw new Error(error.message);
  }
  ...
  async getLeaderboard(limit: number = 5): Promise<{name: string, score: number, city: string}[]> {
    const { data, error } = await supabase
      .from('game_results')
      .select('score, users(name, city)')
  ...
  async getUserUnusedCodes(userId: string): Promise<PromoCode[]> {
    const { data, error } = await supabase
        .from('promo_codes')
        .select('code, is_used, is_issued, assigned_to, generated_at, invoice_number, purchase_amount, is_staff_code')
  ...
  async getAdminStats() {
    ...
    const { data: codes } = await supabase.from('promo_codes').select('code, is_used, is_issued, assigned_to, generated_at, invoice_number, purchase_amount, is_staff_code, users(name, phone)').order('generated_at', { ascending: true });
    const gameConfig = await this.getGameConfig();

    const usedCodesCount = codes?.filter((c: any) => c.is_used).length || 0;

    return {
        users: (users || []).map(u => this.mapUserFromDB(u)),
        prizesConfig: this.mergePrizes(prizes || []),
        allCodes: (codes || []).map((c: any) => ({
            code: c.code,
            isUsed: c.is_used,
            isIssued: c.is_issued,
            assignedTo: c.assigned_to,
            assignedToName: c.users?.name,
            assignedToPhone: c.users?.phone,
            generatedAt: c.generated_at,
            invoiceNumber: c.invoice_number,
            purchaseAmount: c.purchase_amount,
            isStaffCode: c.is_staff_code
  ...
  async uploadCodeRequest(userId: string, photoFile: File) {
    const { data: compressedData, sizeKb } = await compressImage(photoFile);
    const { error } = await supabase.from('code_requests').insert({
        user_id: userId,
        photo_data: compressedData,
        photo_size_kb: sizeKb,
        status: 'pending'
    });
    if (error) throw new Error(error.message);
  }

  async getCodeRequests(status: 'pending' | 'approved' | 'rejected' | 'all' = 'pending'): Promise<any[]> {
      let query = supabase.from('code_requests').select('*, users(name, phone, city, claimed_prizes, max_purchase_tier)');
      if (status !== 'all') {
      ...
      return data.map((r: any) => ({
          ...
          userPhone: r.users?.phone,
          userCity: r.users?.city,
          userPrizes: r.users?.claimed_prizes || [],
          userTier: r.users?.max_purchase_tier,
          photoData: '', 
          photoSizeKb: r.photo_size_kb || 0,
          status: r.status,
  ...
  async getRequestPhoto(requestId: string): Promise<string | null> {
      const { data, error } = await supabase.from('code_requests').select('photo_data').eq('id', requestId).single();
  ...
  async getUserRequestHistory(userId: string): Promise<CodeRequest[]> {
    const { data, error } = await supabase
        .from('code_requests')
        .select('id, user_id, status, admin_comment, invoice_number, purchase_amount, codes_issued, created_at, photo_size_kb')
        .eq('user_id', userId)
  ...
  async processCodeRequest(requestId: string, action: 'approve' | 'reject', payload: { qty?: number, invoice?: string, amount?: number, comment?: string, userId?: string, tier?: PrizeTier }) {
      if (action === 'approve') {
          if (!payload.qty || !payload.invoice || !payload.userId) throw new Error("Missing data for approval");
          await this.issueCodesToUser(payload.userId, payload.qty, payload.invoice, payload.amount || 0, payload.tier);
  ...
  async issueCodesToUser(userId: string, qty: number, invoice: string, amount: number, selectedTier?: PrizeTier, isStaff: boolean = false) {
      const { data: user } = await supabase.from('users').select('max_purchase_tier').eq('id', userId).single();
      const currentTier = (user?.max_purchase_tier as PrizeTier) || 'BASIC';
      
      if (selectedTier) {
          const tiers: PrizeTier[] = ['BASIC', 'BRONZE', 'SILVER', 'GOLD', 'DIAMOND'];
          if (tiers.indexOf(selectedTier) > tiers.indexOf(currentTier)) {
              await supabase.from('users').update({ max_purchase_tier: selectedTier }).eq('id', userId);
          }
      }
  ...
  async getDeliveryUsers(): Promise<User[]> {
      const { data, error } = await supabase.from('users').select('*').eq('delivery_requested', true).order('registered_at', { ascending: false });
      if (error) return [];
      return data.map(u => this.mapUserFromDB(u));
  }

  async completeDelivery(userId: string) {
      // 1. Snapshot and Archive
      const { data: user } = await supabase.from('users').select('*').eq('id', userId).single();
      if (user && user.claimed_prizes?.length > 0) {
          await supabase.from('completed_orders').insert({
              user_id: user.id,
              user_name: user.name,
              user_phone: user.phone,
              user_city: user.city,
              prizes: user.claimed_prizes,
              completed_at: new Date().toISOString()
          });
      }
      // 2. Reset Status
      await supabase.from('users').update({ delivery_requested: false }).eq('id', userId);
  }

  async getCompletedOrders(): Promise<any[]> {
      const { data, error } = await supabase.from('completed_orders').select('*').order('completed_at', { ascending: false });
      if (error) return [];
      return data;
  }

  async deleteCompletedOrder(orderId: string) {
      await supabase.from('completed_orders').delete().eq('id', orderId);
  }
}
